...
#    将x与y拼接在一起，同时将x部分mask掉
    def merge_and_mask(group_lst):
        ...
        for i in range(len(group_lst['input_id_x'])):
            ...
            #attention的部分是前面的用1？
            attention.append([1]*(len(src)))
        group_lst['input_ids'] = lst
        group_lst['input_mask'] = mask
        group_lst['input_x_attention_mask'] = attention
        return group_lst
    ...
    def pad_function(group_lst):
        ...
        group_lst['input_id_x'] = _collate_batch_helper(group_lst['input_id_x'], vocab_dict.pad_token_id, max_length)
        group_lst['input_x_attention_mask'] = _collate_batch_helper(group_lst['input_x_attention_mask'], 0, max_length) #是把后面的补0还是前面的补0？
        return group_lst
...
def get_corpus(data_args, seq_len, split='train', loaded_vocab=None):
    ...
    if data_args.dataset == 'commongen' or data_args.dataset == 'yelp':
        sentence_lst = {'src':[], 'trg': []} 
        if split == 'train':
            print('### Loading form the TRAIN set...')
            path = f'{data_args.data_dir}/{data_args.dataset}.train.jsonl'
        elif split == 'valid':
            print('### Loading form the VALID set...')
            path = f'{data_args.data_dir}/{data_args.dataset}.dev.jsonl'
        elif split == 'test':
            print('### Loading form the TEST set...')
            path = f'{data_args.data_dir}/{data_args.dataset}.test.jsonl'
        else:
            assert False, "invalid split for dataset"

        with open(path, 'r') as f_reader:
            for row in f_reader:
                for i in range(len(json.loads(row)['scene'])):
                    sentence_lst['src'].append(json.loads(row)['concept_set'].strip())
                    sentence_lst['trg'].append(json.loads(row)['scene'][i].strip())
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with torch.no_grad():
            ...
            out_kwargs['input_x_attention_mask'] = np.array(self.text_datasets['train'][idx]['input_x_attention_mask'])
