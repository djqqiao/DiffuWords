...
from diffuwords.contranstive_loss import info_nce
...
class GaussianDiffusion:
    ...
    def training_losses_seq2seq(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        :param x_start: the [N x C x ...] tensor of inputs. # not used unless fixing the input embeddings, torch.Size([B, 128, 128])
        ...
        # input_x = model_kwargs.pop('input_id_x').to(t.device)
        # input_ids_attention_mask = model_kwargs.pop('input_x_attention_mask').to(t.device)
        ...
        # decoder_nll = self._token_discrete_loss(model_output, get_logits, input_ids_x) # embedding regularization
        ...
        query = x_start_mean.view(x_start_mean.shape[0],-1)
        # query = encoded_input_x.view(encoded_input_x.shape[0],-1)

        # pos_key1 = model_out_x_start.view(model_out_x_start.shape[0],-1)

        # pos_key2 = encoded_input_x.view(encoded_input_x.shape[0],-1)

        # pos_keys = th.concat(pos_key1,pos_key2)

        pos_keys = model_out_x_start.view(model_out_x_start.shape[0],-1)

        con_loss = info_nce(query,pos_keys,negative_keys=None,temperature=0.1)

        # terms["loss"] = terms["mse"] + decoder_nll + tT_loss

        terms["loss"] = terms["mse"]  + tT_loss + decoder_nll + 0.01*con_loss
